...
- 一個父節點下最多五個子節點。要開第六個時，先看這六個子問題能不能依主題分成兩群；能就拆成 A、B 兩個**同層的根**（例：A 改名「A API 面」留 A.1–A.3，B 新增「B 資料面」接手原 A.4–A.6 並重編為 B.1–B.3），各自數進度（`A 2/3`、`B 0/3`）。分不出兩群，代表子問題切太細，先合併相近的，不硬拆。B 不是 A 的子節點，不繼承 A 的編號。一個檔最多三個根，超過就該寫設計文件了。
- 節點文字 15 字以內，只寫問題本身。理由、數據、選項不進圖，進清單。
- 子問題解決時：節點改 ✅、結論一行寫進「已解決」、目前位置彈回父節點、更新進度數字。
- 所有子節點都 ✅，父節點也改 ✅。
- 只記使用者說過的結論，不自行補完。提案要標「提案」，不當結論。
- 狀態列、清單標題、節點文字、回覆一律用使用者的語言。英文對應：`Progress n/m ｜ Now: X ｜ Next: …`；清單 `Needs decision` / `Blocked` / `Solved`。本檔的中文範例只是範例。
...
`docs/superpowers/handoff/` 下未完成的檔超過三個時，多問一句：哪一個可以先結案移到 `done/`？
