...
- **Guardrails**: one 🔶 at a time, depth ≤ 2, ≤ 5 children per node, node text ≤ 15 chars, only conclusions the user actually stated.
...
- **護欄**：同時只有一個 🔶、深度最多兩層、一個父節點最多五個子節點、節點文字 15 字內、只記使用者說過的結論。
